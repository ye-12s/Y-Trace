...
#define ULOG_ASYNC_OUTPUT_STORE_LINES                                          \
    (ULOG_ASYNC_OUTPUT_BUF_SIZE * 3 / 2 / ULOG_LINE_BUF_SIZE)
#endif
...
#else
        rt_kprintf("Error: Current mode not supported run in ISR. Please "
                   "enable ULOG_USING_ISR_LOG.\n");
        return NULL;
#endif
    }
}

RT_WEAK rt_size_t ulog_formater(char *log_buf, rt_uint32_t level,
    const char *tag, rt_bool_t newline, const char *format, va_list args) {
    ...
    if (color_output_info[level]) {
        ...
        log_len +=
            ulog_strcpy(log_len, log_buf + log_len, color_output_info[level]);
    }
#endif /* ULOG_USING_COLOR */
    ...
    /* add time info */
    {
        ...
        rt_snprintf(log_buf + log_len, ULOG_LINE_BUF_SIZE - log_len,
            "%02d-%02d %02d:%02d:%02d.%03d", tm->tm_mon + 1, tm->tm_mday,
            tm->tm_hour, tm->tm_min, tm->tm_sec, rt_tick_get() % 1000);
#else
        rt_snprintf(log_buf + log_len, ULOG_LINE_BUF_SIZE - log_len,
    ...
    /* add level info */
    log_len +=
        ulog_strcpy(log_len, log_buf + log_len, level_output_info[level]);
...
#if defined(ULOG_OUTPUT_TIME) || defined(ULOG_OUTPUT_LEVEL) ||                 \
    defined(ULOG_OUTPUT_TAG)
        log_len += ulog_strcpy(log_len, log_buf + log_len, " ");
#endif
        ...
        if (rt_interrupt_get_nest() == 0) {
            rt_size_t name_len =
                rt_strnlen(rt_thread_self()->name, RT_NAME_MAX);
    ...
    fmt_result = vsnprintf(
        log_buf + log_len, ULOG_LINE_BUF_SIZE - log_len, format, args);
#else
    fmt_result = rt_vsnprintf(
        log_buf + log_len, ULOG_LINE_BUF_SIZE - log_len, format, args);
...
void ulog_output_to_all_backend(rt_uint32_t level, const char *tag,
    rt_bool_t is_raw, const char *log, rt_size_t size) {
    rt_slist_t *node;
    ...
    for (node = rt_slist_first(&ulog.backend_list); node;
...
static void do_output(rt_uint32_t level, const char *tag, rt_bool_t is_raw,
    const char *log_buf, rt_size_t log_len) {
#ifdef ULOG_USING_ASYNC_OUTPUT
    ...
    log_blk = rt_rbb_blk_alloc(ulog.async_rbb,
        RT_ALIGN(sizeof(struct ulog_frame) + log_len, RT_ALIGN_SIZE));
    if (log_blk) {
    ...
    if (rt_interrupt_get_nest() == 0) {
        ...
        extern void ulog_console_backend_output(struct ulog_backend * backend,
            rt_uint32_t level, const char *tag, rt_bool_t is_raw,
 ...
 */
void ulog_voutput(rt_uint32_t level, const char *tag, rt_bool_t newline,
...
#else
    extern rt_size_t syslog_formater(char *log_buf, rt_uint8_t level,
        const char *tag, rt_bool_t newline, const char *format, va_list args);
 ...
 */
void ulog_hexdump(
    const char *tag, rt_size_t width, rt_uint8_t *buf, rt_size_t size) {
#define __is_print(ch) ((unsigned int)((ch) - ' ') < 127u - ' ')
    ...
    if (LOG_LVL_DBG > ulog.filter.level ||
        LOG_LVL_DBG > ulog_tag_lvl_filter_get(tag)) {
        return;
    }
#else
    ...
    for (i = 0, log_len = 0; i < size; i += width) {
        ...
        fmt_result = rt_snprintf(log_buf + log_len, ULOG_LINE_BUF_SIZE,
            "%04X-%04X: ", i, i + width - 1);
        /* calculate log length */
        if ((fmt_result > -1) && (fmt_result <= ULOG_LINE_BUF_SIZE)) {
            log_len += fmt_result;
        ...
        for (j = 0; j < width; j++) {
            if (i + j < size) {
                rt_snprintf(
                    dump_string, sizeof(dump_string), "%02X ", buf[i + j]);
            } else {
        ...
        for (j = 0; j < width; j++) {
            if (i + j < size) {
                rt_snprintf(dump_string, sizeof(dump_string), "%c",
...
int ulog_tag_lvl_filter_set(const char *tag, rt_uint32_t level) {
    ...
    for (node = rt_slist_first(ulog_tag_lvl_list_get()); node;
        node = rt_slist_next(node)) {
    ...
    if (tag_lvl) {
        ...
        if (level != LOG_FILTER_LVL_ALL) {
            ...
            tag_lvl = (ulog_tag_lvl_filter_t)rt_malloc(
                sizeof(struct ulog_tag_lvl_filter));
            if (tag_lvl) {
...
static void ulog_tag_lvl(uint8_t argc, char **argv) {
    if (argc > 2) {
        if ((atoi(argv[2]) <= LOG_FILTER_LVL_ALL) && (atoi(argv[2]) >= 0)) {
            ...
            rt_kprintf(
                "Please input correct level (0-%d).\n", LOG_FILTER_LVL_ALL);
        }
    } else {
...
#ifndef ULOG_USING_SYSLOG
    const char *lvl_name[] = {"Assert ", "Error  ", "Error  ", "Error  ",
        "Warning", "Info   ", "Info   ", "Debug  "};
#endif
    const char *tag = ulog_global_filter_tag_get(),
               *kw = ulog_global_filter_kw_get();
    rt_slist_t *node;
    ulog_tag_lvl_filter_t tag_lvl = NULL;
...
rt_err_t ulog_backend_register(
    ulog_backend_t backend, const char *name, rt_bool_t support_color) {
    rt_base_t level;
...
void ulog_async_output(void) {
    ...
    while ((log_blk = rt_rbb_blk_get(ulog.async_rbb)) != NULL) {
        ...
        if (log_frame->magic == ULOG_FRAME_MAGIC) {
            /* output to all backends */
            ulog_output_to_all_backend(log_frame->level, log_frame->tag,
...
int ulog_init(void) {
    ...
    ulog.async_rbb =
        rt_rbb_create(RT_ALIGN(ULOG_ASYNC_OUTPUT_BUF_SIZE, RT_ALIGN_SIZE),
    ...
    /* async output thread */
    ulog.async_th = rt_thread_create("ulog_async", async_output_thread_entry,
        &ulog, ULOG_ASYNC_OUTPUT_THREAD_STACK,
        ULOG_ASYNC_OUTPUT_THREAD_PRIORITY, 20);
    if (ulog.async_th == NULL) {
        rt_kprintf(
            "Error: ulog init failed! No memory for async output thread.\n");
        rt_mutex_detach(&ulog.output_locker);
