...
sd_error_status_type sd_switch(uint32_t mode, uint32_t group, uint8_t value,
                               uint8_t* rsp);
...
sd_error_status_type sd_init(void) {
  ...
  /* enable gpio, iomux and sdio clock */
  ...
  /* 先配置引脚复用，再配置GPIO！
   * SDIO2_GMUX_0001: PC4(CLK), PC5(CMD), PA4(D0), PA5(D1), PA6(D2), PA7(D3) */
  gpio_pin_remap_config(SDIO2_GMUX_0001, TRUE);

  /* configure PA4-PA7: SDIO2 D0, D1, D2, D3 */
  gpio_init_struct.gpio_drive_strength = GPIO_DRIVE_STRENGTH_STRONGER;
  gpio_init_struct.gpio_mode = GPIO_MODE_MUX;
  gpio_init_struct.gpio_out_type = GPIO_OUTPUT_PUSH_PULL;
  gpio_init_struct.gpio_pins =
      GPIO_PINS_4 | GPIO_PINS_5 | GPIO_PINS_6 | GPIO_PINS_7;
  gpio_init_struct.gpio_pull = GPIO_PULL_UP;  /* 数据线需要上拉 */
  gpio_init(GPIOA, &gpio_init_struct);

  /* configure PC4: SDIO2 CLK, PC5: SDIO2 CMD */
  gpio_init_struct.gpio_pins = GPIO_PINS_4 | GPIO_PINS_5;
  gpio_init_struct.gpio_pull = GPIO_PULL_UP;  /* CLK和CMD需要上拉 */
  ...
  if (status == SD_OK) {
    ...
    rt_kprintf("[SDIO] Before card_info_get: card_type=%d\n", card_type);
    status = sd_card_info_get(&sd_card_info);
    rt_kprintf("[SDIO] After card_info_get: card_type=%d, capacity=%d MB\n", 
               card_type, (int)(sd_card_info.card_capacity / (1024 * 1024)));
  }
  ...
  if (status == SD_OK && ((SDIO_STD_CAPACITY_SD_CARD_V1_1 == card_type) ||
    ...
    sd_error_status_type scr_status = scr_find();
    if (scr_status != SD_OK) {
      rt_kprintf("[SDIO] SCR read failed, assuming card supports 4-bit mode\n");
      /* SCR读取失败，为SDHC卡设置默认值（假设支持4-bit） */
      if (card_type == SDIO_HIGH_CAPACITY_SD_CARD) {
        sd_card_info.sd_scr_reg.sd_bus_width = 1;  /* 假设支持4-bit */
        sd_card_info.sd_scr_reg.sd_spec = 2;       /* SD 2.0 */
      }
      /* 不设置status为错误，继续初始化 */
    }
  }
  ...
  if (status == SD_OK) {
    /* try to set normal speed mode (optional for old SD cards) */
    sd_error_status_type speed_status = speed_change(0);
    if (speed_status != SD_OK) {
      rt_kprintf("[SDIO] Speed switching not supported, using default mode\n");
      /* 不修改status，让初始化继续 */
    } else {
      rt_kprintf("[SDIO] Speed mode configured successfully\n");
    }
  }
  ...
  if (status == SD_OK) {
    /* Try to set 4-bit wide bus mode */
    sd_error_status_type bus_status = sd_wide_bus_operation_config(SDIO_BUS_WIDTH_D4);
    if (bus_status == SD_REQ_NOT_APPLICABLE) {
      rt_kprintf("[SDIO] Using 1-bit bus mode (card limitation)\n");
      /* 直接配置SDIO控制器为1-bit模式，不发送命令到卡 */
      sdio_bus_width_config(SDIOx, SDIO_BUS_WIDTH_D1);
      /* 不设置status为错误，继续初始化 */
    } else if (bus_status != SD_OK) {
      rt_kprintf("[SDIO] Failed to set bus width: %d\n", bus_status);
      status = bus_status;
    } else {
      rt_kprintf("[SDIO] 4-bit bus mode configured successfully\n");
    }
  }
...
sd_error_status_type sd_power_on(void) {
  ...
  if (SD_OK == status) {
    ...
    /* 检查OCR响应中的CCS位(bit 30)来判断是否为SDHC/SDXC */
    if (response & SD_HIGH_CAPACITY) {
      card_type = SDIO_HIGH_CAPACITY_SD_CARD;
      rt_kprintf("[SDIO] Detected SDHC/SDXC card (High Capacity)\n");
    } else {
      rt_kprintf("[SDIO] Detected SD v2.0 Standard Capacity card\n");
    }
  }
  /* mmc card */
  else {
...
sd_error_status_type sd_card_info_get(sd_card_info_struct_type* card_info) {
  ...
  rt_kprintf("[SDIO] Calculating capacity for card_type=%d\n", card_type);
  
  if ((card_type == SDIO_STD_CAPACITY_SD_CARD_V1_1) ||
      (card_type == SDIO_STD_CAPACITY_SD_CARD_V2_0) ||
      (card_type == SDIO_MULTIMEDIA_CARD)) {
    rt_kprintf("[SDIO] Using standard capacity formula\n");
    ...
    rt_kprintf("[SDIO] Using SDHC/SDXC capacity formula\n");
    /* byte 7 */
    tmp = (uint8_t)(csd_table[1] & 0x000000FF);
    ...
    rt_kprintf("[SDIO] SDHC: device_size=0x%X, capacity=%d MB\n",
               card_info->sd_csd_reg.device_size,
               (int)(card_info->card_capacity / (1024 * 1024)));
  }
...
sd_error_status_type sd_wide_bus_operation_config(sdio_bus_width_type mode) {
  ...
  if (card_type == SDIO_MULTIMEDIA_CARD ||
      ...
      card_type == SDIO_HIGH_CAPACITY_MMC_CARD) {
    status =
        mmc_switch(EXT_CSD_CMD_SET_NORMAL, EXT_CSD_BUS_WIDTH, (uint8_t)mode);
 ...
 * @retval sd_error_status_type: sd card error code.
 */
sd_error_status_type sd_block_read(uint8_t* buf, long long addr,
                                   uint16_t blk_size) {
  sd_error_status_type status = SD_OK;
  uint32_t response = 0;
  uint8_t power;
 ...
 * @retval sd_error_status_type: sd card error code.
 */
sd_error_status_type sd_block_write(const uint8_t* buf, long long addr,
                                    uint16_t blk_size) {
  sd_error_status_type status = SD_OK;
...
sd_error_status_type sd_mult_blocks_write(const uint8_t* buf, long long addr,
  ...
  sdio_command_init_struct.argument = nblks;
 ...
 * @retval sd_error_status_type: sd card error code.
 */
sd_error_status_type mmc_stream_read(uint8_t* buf, long long addr,
                                     uint32_t len) {
  uint32_t response = 0;
 ...
 * @retval sd_error_status_type: sd card error code.
 */
sd_error_status_type mmc_stream_write(uint8_t* buf, long long addr,
                                      uint32_t len) {
  sd_error_status_type status = SD_OK;
  uint32_t response = 0;
 ...
 * @retval none.
 */
void SDIO1_IRQHandler(void) { sd_irq_service(); }
 ...
 * @retval none.
 */
void SDIO2_IRQHandler(void) { sd_irq_service(); }
...
sd_error_status_type sd_irq_service(void) {
  ...
  sdio_interrupt_enable(
      SDIOx,
      (SDIO_DTFAIL_INT | SDIO_DTTIMEOUT_INT | SDIO_DTCMP_INT | SDIO_TXBUFH_INT |
       SDIO_RXBUFH_INT | SDIO_TXERRU_INT | SDIO_RXERRO_INT | SDIO_SBITERR_INT),
      FALSE);
  return (SD_OK);
}
...
sd_error_status_type get_ext_csd(void) {
  ...
  /* 命令响应成功后，启用数据状态机开始接收数据 */
  sdio_data_state_machine_enable(SDIOx, TRUE);

  sts_reg = SDIOx->sts;
  uint32_t timeout = 0x1FFFFFF; /* 添加超时保护 */
  ...
  while (
    ...
    if (sdio_flag_get(SDIOx, SDIO_RXBUF_FLAG) != RESET) {
      *(tmp_ext_csd + index) = sdio_data_read(SDIOx);
      index++;
      timeout = 0x1FFFFFF; /* 有数据活动时重置超时 */
    }
    
    if (--timeout == 0) {
      rt_kprintf("[SDIO] get_ext_csd timeout, sts=0x%08X, index=%d\n", sts_reg, index);
      return SD_DATA_TIMEOUT;
    }
  ...
  if ((SDIO_STD_CAPACITY_SD_CARD_V1_1 == card_type) ||
    ...
    if ((switch_sts[16] & 0xF) != speed) {
      return SD_ERROR;
             ...
             card_type == SDIO_HIGH_CAPACITY_MMC_CARD) {
    status =
        mmc_switch(EXT_CSD_CMD_SET_NORMAL, EXT_CSD_BUS_WIDTH, (uint8_t)speed);
